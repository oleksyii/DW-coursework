...
@app.get("/users")
async def get_users(order_by: str = 'name', order_direction: str = 'ASC', num: int = 100):
...
async def get_countries():
    await connect_to_db()
    countries = await c.fetch_countries(globals['db_conn'])
    return countries
...
@app.get("/games/{page}")
async def get_items(page: int, passedLimit: Optional[int] = Query(10, ge=1, le=100)):
    await connect_to_db()
